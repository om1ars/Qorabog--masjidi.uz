  ...
  const handleSearch = () => {
    return quranData.filter(
      (output) =>
        ...
        output.englishNameTranslation.toLowerCase().include.(search)
    );
  };
              ...
              <>
                <Table>
                  <TableHead style={{ backgroundColor: "#eebc10" }}>
                    <TableRow>
                      {[
                        "Sura nomi. Suralar faqat ingliz tilida. Uzbek tilida ma'lumotlar mavjud emas.Buning uchun uzur so'raymiz",
                        "Sura nomi",
                        "Manosi (inglizchada)",
                        "Nozil bo'lgan joyi",
                      ].map((head) => (
