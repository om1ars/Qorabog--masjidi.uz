...
export const SingleChapter = ({ quranData, loading, setLoading }) => {
  ...
  const handleSearch = () => {
    return quranData.filter((output) =>
      output.name.toLowerCase().includes(search)  || output.englishName.toLowerCase().includes(search)
    );
  };
              ...
              <>
                <Table>
                  <TableHead style={{ backgroundColor: "#eebc10" }}>
                    <TableRow>
                      {["Sura nomi. Suralar faqat ingliz tilida. Uzbek tilida ma'lumotlar mavjud emas.Buning uchun uzur so'raymiz", "Sura nomi", "Manosi (inglizchada)", "Nozil bo'lgan joyi"].map(
                        (head) => (
                          <TableCell
                            style={{
                              color: "black",
